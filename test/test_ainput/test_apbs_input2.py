...
from apbs.ainput.easyjson3 import parse
...
class TestApbsLegacyInput:
    def test_basic(self):
        ...
        for idx, section in enumerate(config["READ"]):
            for jdx, file in enumerate(config["READ"][idx]["mol"]["pqr"]):
                print(f"FILE: {idx} {jdx} {file}")
    ...
    def test_query(self):
        ...
        filename = os.path.join(os.path.dirname(__file__), "junk.json")
        ...
        print(type(config))
        print(config)
        config = parse(json.dumps(config))
        a = config.READ.mol.pqr[...].file
        b = config.ELEC[...]
        for idx, filename in enumerate(a):
            print(f"A idx: {idx}, file: {filename}")
        print(b)
