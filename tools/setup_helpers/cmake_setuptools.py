...
class CMakeBuild(build_ext):
    ...
    def build_extension(self, ext):
        ...
        cmake_args += ["-DCMAKE_INSTALL_PREFIX=" + os.path.abspath(sys.prefix)]
        ...
        print("-- Configuring")
        subprocess.run(["cmake", ext.sourcedir] + cmake_args, cwd=build_dir)

        print("-- Building")
        subprocess.run(["cmake", "--build", "."] + build_args, cwd=build_dir)


class CMakeTest(build_ext):
    def run(self):
        try:
            out = subprocess.check_output(["ctest", "--version"])
        except OSError:
            raise RuntimeError("CTest must be installed to test APBS: ")

        ctest_args = [
            "-C Release --verbose --output-on-failure",
        ]

        print("-- Testing")
        subprocess.run(["ctest"] + ctest_args, cwd=build_dir)